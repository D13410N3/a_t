...
{{- else }}
    value: "jdbc:postgresql://{{ template "mainDBHost" . }}:{{ template "mainDBPort" . }}/{{ template "mainDBName" . }}?sslmode={{ template "mainDBSSL" . }}"
{{- end }}
  - name: SPRING_DATASOURCE-UPLOADER_USERNAME
    valueFrom:
      secretKeyRef:
        name: {{ template "testops.secret.name" . }}
        key: "testopsDbUser"
  - name: SPRING_DATASOURCE-UPLOADER_PASSWORD
    valueFrom:
      secretKeyRef:
        name: {{ template "testops.secret.name" . }}
        key: "testopsDbPass"
  - name: SPRING_DATASOURCE-UPLOADER_HIKARI_MAXIMUMPOOLSIZE
    value: "{{ .Values.datasources.uploaderDatasource.appMaxDBConnection }}"
  - name: SPRING_DATASOURCE-UPLOADER_HIKARI_CONNECTIONTIMEOUT
...
{{- range $key, $value := .Values.widgetsCache.widgetsCustom }}
  - name: ALLURE_CACHE-TTL_WIDGETS-CUSTOM_{{ $key }}
    value: {{ $value }}
{{- end }}
{{- end }}
